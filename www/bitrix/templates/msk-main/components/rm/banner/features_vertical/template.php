...
?>
<div class="bSideFeatures">
	<?
	foreach ($arResult['ITEMS'] as $item) {
		$alt = htmlspecialchars($item['NAME']);

		$item['NAME']
			= mb_strlen($item['~NAME']) <= 16 ? ('<span class="bFeatures__eBottomText">' . $item['NAME'] . '</span>')
			: $item['NAME'];
        ...
        <div class="bFeatures__eItem bFeatures__eItem__mSide_position">
            <div class="bFeatures__eTopLine">
                ...
                <img src="<?=$item['DETAIL_PICTURE']['SRC']?>" class="bFeatures__eImage <?=$imgClass?>" alt="<?=$alt?>"/></div>
