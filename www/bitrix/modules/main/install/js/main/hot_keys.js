...
/*		erase candidat
		var btnDefault = new BX.CWindowButton({
			...
			'action': function() { if(confirm(_this.MesDefaultConfirm)) { _this.DelAll(); _this.SetDefault(); } }
		});

		var btnDelAll = new BX.CWindowButton({
			'title': this.MesDelAll,
			'action': function() { if(confirm(_this.MesDelConfirm)) _this.DelAll(); }
		});

		var btnExport = new BX.CWindowButton({
			'title': this.MesExport,
			'action': function() { _this.Export(); }
		});

		var btnImport = new BX.CWindowButton({
			'title': this.MesImport,
			'action': function() { _this.Import(); }
		});
*/
		...
		BX.addCustomEvent(obWnd, 'onWindowClose', function(obWnd) {
																	var dialog = BX.findChild(document,{attribute: {'class':'bx-core-window bx-core-dialog'}},true);

																	if(!dialog)
																		dialog = _this.hk_getElementsByClass("bx-core-window bx-core-dialog",document,"div")[0];
		...
		var dialog_head = BX.findChild(document, {attribute: {'class': 'dialog-head'}}, true );

		if(dialog_head)
		{
			dialog_head.appendChild(hk_menu_div);
			BX.findChild(document, {attribute: {'class': 'bx-core-dialog-content'}}, true ).style.marginTop="45px";
		}
		else // ie quirck mode
		{
			this.hk_getElementsByClass("dialog-head")[0].appendChild(hk_menu_div);
	...
	this.Import = function()
	{
		...
		var formText = 	'<form action="/bitrix/admin/hot_keys_act.php?hkaction=import" method="post" enctype="multipart/form-data" target="upload_iframe" id="hk_import_form" name="hk_import_form">'+
      					...
      					'<input type="file" name="bx_hk_filename" id="bx_hk_filename">'+
      					'</form>'+
