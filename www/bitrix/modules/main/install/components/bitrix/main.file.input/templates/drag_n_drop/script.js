function BlogBFileDialog(arParams)
{
	...
	BX.loadScript('/bitrix/js/main/file_upload_agent.js', BX.delegate(function() {
	...
	BX.loadScript('/bitrix/js/main/core/core_dd.js', BX.delegate(function() {
		if (BX.type.isElementNode(this.controller) && this.controller.parentNode && this.controller.parentNode.parentNode)
		{
			...
			if (this.dropbox && this.dropbox.supported() && BX.ajax.FormData.isSupported()) {
				...
				//var parentLogContainer = BX.findParent(target, {'tag': 'div', 'property' : {'id' : 'sonet_log_microblog_container'}});
			}
		}
	}, this));
}
...
BlogBFileDialogUploader.prototype.Callback = function(files, uniqueID)
{
	if (files.length > 0) {
		for(var i = 0; i < files.length; i++) {
			...
			result.element_content_type = files[i].content_type;
			result.element_image = ((!!files[i].img_thumb_src) ? files[i].img_thumb_src: '');
			result.element_thumbnail = ((!!files[i].img_source_src) ? files[i].img_source_src: '');
