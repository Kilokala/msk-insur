...
?>
				<tr class="file-inline-file" id="wd-doc<?=$elementID?>" onclick="BfileFD<?=$uid?>.agent.StopUpload(this);">
					<td class="files-name">
					...
					<td class="files-storage">
						<div class="files-storage-block">&nbsp;
							...
							<input id="file-doc<?=$elementID?>" type="hidden" name="<?=htmlspecialchars($controlNameFull)?>" value="<?=$elementID?>" />
						</div>
					</td>
				</tr>
<?
		...
		if (! BX.browser.IsIE())
		{
			...
			function BfileUnbindDispatcher<?=$uid?>()
			{
				if (!! bfDisp<?=$uid?>) {
					...
					setTimeout(BfileUnbindDispatcher<?=$uid?>, 300);
				}
			}
		}
