...
if (is_array($arResult["ActiveFeatures"]) && count($arResult["ActiveFeatures"]) > 0):
	...
	{	
		...
		else
		{
			...
			foreach($flt_event_id as $key => $val)
				$flt_event_id[$key] = htmlspecialchars($val);
	...
	{
		$_REQUEST["flt_date_datesel"] = htmlspecialchars($_REQUEST["flt_date_datesel"]);
	...
	else
	{
		...
		{
			$_REQUEST["flt_date_from"] = htmlspecialchars($_REQUEST["flt_date_from"]);
			$arParams["LOG_DATE_FROM"] = trim($_REQUEST["flt_date_from"]);
		}
		if (array_key_exists("flt_date_to", $_REQUEST))
		{
			$_REQUEST["flt_date_to"] = htmlspecialchars($_REQUEST["flt_date_to"]);
		...
		while($row = $results->NavNext(true, "a_"))     
		{
			if (!in_array($row['USER_ID'], array_keys($arUsersTmp)))
			{
				...
				if($arUser = $rsUser->GetNext())
				{
					...
					$arUserInfo["FULL_NAME"] = $arUser["NAME"].(strlen($arUser["NAME"])<=0 || strlen($arUser["LAST_NAME"])<=0?"":" ").$arUser["LAST_NAME"];
					if (empty($arUserInfo["FULL_NAME"]))
						$arUserInfo["FULL_NAME"] = $arUser["LOGIN"]; 
