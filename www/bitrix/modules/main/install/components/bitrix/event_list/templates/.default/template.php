			...
			<table cellspacing="0" border="0">
				...
				<div class="event-list-filter-date-title"><?=GetMessage("EVENT_LIST_FILTER_DATE");?>:</div>
					...
					<span class="filter-field filter-day-interval" style="display:none" id="flt_date_day_span">
						<input type="text" name="flt_date_days" value="<?=htmlspecialchars($_REQUEST["flt_date_days"])?>" class="filter-date-days" size="2" /> <?echo GetMessage("EVENT_LIST_DATE_FILTER_DAYS")?>
					</span>
					<span class="filter-date-interval filter-date-interval-after filter-date-interval-before">
						<span class="filter-field filter-date-interval-from" style="display:none" id="flt_date_from_span">
							<input type="text" name="flt_date_from" value="<?=(array_key_exists("LOG_DATE_FROM", $arParams) ? htmlspecialchars($arParams["LOG_DATE_FROM"]) : "")?>" class="filter-date-interval-from" /><?
							$APPLICATION->IncludeComponent(
								...
								array(
									...
									"INPUT_VALUE"	=> (array_key_exists("LOG_DATE_FROM", $arParams) ? htmlspecialchars($arParams["LOG_DATE_FROM"]) : ""),
						...
						<span class="filter-field filter-date-interval-to" style="display:none" id="flt_date_to_span">
							<input type="text" name="flt_date_to" value="<?=(array_key_exists("LOG_DATE_TO", $arParams) ? htmlspecialchars($arParams["LOG_DATE_TO"]) : "")?>" class="filter-date-interval-to" /><?
							$APPLICATION->IncludeComponent(
								"bitrix:main.calendar",
								"",
								array(
									...
									"INPUT_VALUE"	=> (array_key_exists("LOG_DATE_TO", $arParams) ? htmlspecialchars($arParams["LOG_DATE_TO"]) : ""),
									"FORM_NAME"		=> "log_filter",
								),
								$component,
								array("HIDE_ICONS"	=> true)
				...
				<span class="event-list-item-body">
					...
					<div class="event-list-item-content">												
						<div class=" event-list-message-show">
							<?
								if ($arEvent["eventURL"] != "")
									$eventName = "<a href =".$arEvent['eventURL'].">\"".htmlspecialchars($arEvent["eventName"])."\"</a>";
								else
									$eventName = "\"".htmlspecialchars($arEvent["eventName"])."\"";
