...
class CVulnScanner
{
    ...
    function __construct($file_name, $arParams, $template = '.default', $component_template = '')
    {
        ...
        $this->scan_functions = array_merge($this->vuln_func['XSS'], $this->vuln_func['CODE'], $this->vuln_func['FILE_INCLUDE'], $this->vuln_func['EXEC'], $this->vuln_func['DATABASE'], $this->vuln_func['OTHER']);
    ...
    private function haveUserInput($tokens, $var_declare = true, $function = '')
    {
        $arResult = array();
        ...
        for ($i = 0, $count = count($tokens); $i < $count; $i++) {
            if (is_array($tokens[$i])) {
                ...
                if ($token === T_DOUBLE_COLON || $token === T_OBJECT_OPERATOR) {
                    ...
                    if ($var_declare || $this->scan_functions[$function][0] === 0 || in_array($c_params, $this->scan_functions[$function][0])) {
                        ...
                        if ((is_array($tokens[$i - 1])
                            ...
                            if (!in_array($token_value, array_keys($arResult))) {
                                ...
                                else if (isset($this->variables[$val = $this->getVarName($tokens[$i])])) {
                                ...
                                else if (((isset($this->variables[$token_value]) && $this->variables[$token_value]->have_user_input)) && ($var_declare || !$this->variables[$token_value]->secure || $unsecure)) {
                                    $arResult[$token_value]['have_user_input'] = true;
                ...
                } else if ($cur_brace === -1 && $token === T_STRING
                    && in_array($token_value, $this->sec_func['INSTRING'])
                ) {
                    $unsecure = true;
                    $secure = false;
                    $cur_brace = $braces;
                } else if (!$unsecure && ($token === T_STRING
                    && (
                    ...
                    $braces++;
                    $i++;
                } else if ($token === T_ISSET || ($token === T_STRING && substr($token_value, 0, 3) === 'is_')) {
                    $skip = true;
                }
            } else if ($braces === 1 && $tokens[$i] === ',') {
                $c_params++;
                $skip = false;
            } else if ($tokens[$i] === '(') {
                $braces++;
    ...
    public function process()
    {
        ...
        for ($i = 0, $tokens_count = count($this->tokens); $i < $tokens_count; $i++) {
            ...
            if (is_array($this->tokens[$i])) {
                ...
                elseif (in_array($token, $this->tokens_type['FUNCTIONS']) || ($this->search_xss && !$this->global_xss_ignore && in_array($token, $this->tokens_type['XSS'])) || in_array($token, $this->tokens_type['INCLUDES'])) {
                    ...
                    if (in_array($token, $this->tokens_type['INCLUDES'])) {
                        ...
                        else if ($token == T_INCLUDE_RESULT_MODIFIER) {
                            if (!$this->mp_mode) {
                                $skip = 1;
                                ...
                                } else if (is_file($this->arParams['doc_root_path'] . '/bitrix/templates/' . $component_template . '/components' . $component_path . '/.default/result_modifier.php')) {
                        ...
                        else if ($token == T_INCLUDE_COMPONENTTEMPLATE) {
                            if (!$this->mp_mode) {
                                ...
                                else if ($this->tokens[$i + 1] === '(' && $this->tokens[$i + 2] === ')') {
                                    $skip = 2;
                                }
                                $component_path = array_pop(explode('bitrix/components', dirname($this->current_file)));
                                ...
                                } else if (is_file($this->arParams['doc_root_path'] . '/bitrix/templates/.default/components' . $component_path . '/' . $component_template . '/' . $template_name . '.php')) {
                    ...
                    if (isset($this->scan_functions[$token_value])
                        ...
                        if ($this->tokens[$i + 1] === '(')
                            $result = $this->haveUserInput(array_slice($this->tokens, $i + 2, $this->getBraceEnd($this->tokens, $i + 2) - 1), false, $token_value);
                        else
                            $result = $this->haveUserInput(array_slice($this->tokens, $i + 1, $this->getBraceEnd($this->tokens, $i + 1)), false, $token_value);
                        ...
                        if ($result !== false) {
                            ...
                            if (!empty($tainted_vars)) {
                                ...
                                $vuln = new CVuln($this->scanning_file, $cur_line, $token_value, array_slice($this->tokens, $pos, $this->getBraceEnd($this->tokens, $pos)), $this->dependencies, $tainted_vars, $this->comment);
                                $this->arResult[] = $vuln;
                            }
                ...
                else if (in_array($token, $this->tokens_type['FLOW_CONTROL'])) {
                    $c = 1;
            ...
            for ($i = 0, $count = count($token[3]); $i < $count && ($i < $level || $level === -1); $i++) {
                if (is_array($token[3][$i])) {
                    /*
                                    ...
                                    $res=$this->haveUserInput($token[3][$i],true);
                                    if ($res === false)
    ...
    private function addVariable($var, $id, $line, $start, $end, $comment = '', $custom_tokens = array())
    {

        $tokens = !empty($custom_tokens) ? $custom_tokens : array_slice($this->tokens, $start, $end);
        $tokens_for_scan = !empty($custom_tokens) ? $custom_tokens : array_slice($this->tokens, $start + 2, $end - 2);
        $user_input = $this->haveUserInput($tokens_for_scan);
        ...
        $var_name = self::getVarName($var);
        $this->clearVariables($var_name);
        if ($user_input !== false) {
            $tainted_vars = array();
            foreach ($user_input[1] as $res)
                $tainted_vars[] = $res['var_name'];
            if (!isset($this->variables[$var_name])) {
                $var = new CVariable($var_name);
            } else {
                $var = $this->variables[$var_name];
            }
            $var->have_user_input = true;
            $var->secure = $user_input[0];
            $var->newDeclare($id, $line, $start, $end, $tokens, $this->comment . $comment, $dependencies, $tainted_vars);
            $this->variables[$var_name] = $var;
        } elseif (isset($this->variables[$var_name]))
            unset($this->variables[$var_name]); //TODO: Fix this, with dependency overwritten!
    }
    ...
    private function wrapBraces($tokens, $start, $between, $end)
    {
        ...
        $tokens = array_merge(
            array_slice($tokens, 0, $start), array('{'), array_slice($tokens, $start, $between), array('}'), array_slice($tokens, $end)
        );
    ...
    private function fixTokens($tokens, $component_template = '')
    {
        for ($i = 0; $i < ($max = count($tokens)); $i++) {
            if ($tokens[$i] === '`') {
                ...
                if (is_array($tokens[$i]) && (strtolower($tokens[$i][1]) === 'includecomponenttemplate' || strtolower($tokens[$i][1]) === 'initcomponenttemplate')) {
                    ...
                    $i++;
                } else if (is_array($tokens[$i]) && (strtolower($tokens[$i][1]) == 'includecomponent')) {
                    $tokens[$i][1] = strtolower($tokens[$i][1]);
                    $tokens[$i][0] = T_INCLUDE_COMPONENT;
                } else if (($tokens[$i][0] === T_IF || $tokens[$i][0] === T_ELSEIF || $tokens[$i][0] === T_FOR
                ...
                } else if ($tokens[$i][0] === T_SWITCH && $tokens[$i + 1] === '(') {
                    $braces = 1;
                    $c = 2;
                    while ($braces !== 0) {
                        if ($tokens[$i + $c] === '(') {
                            ...
                            $braces--;
                        } else if (!isset($tokens[$i + $c]) || $tokens[$i + $c] === ';')
                            break;
                        $c++;
                    }
                    if ($tokens[$i + $c] === ':') {
                    ...
                    $tokens = array_merge(
                        array_slice($tokens, 0, $i), array_slice($tokens, $i + $f, $d), array_slice($tokens, $i + 1, $f - 1), array_slice($tokens, $i + $f + $d + 1, count($tokens)));
                }
            }
        }
        return array_values($tokens);
                ...
                if ($tokens[$i][0] === T_VARIABLE
                    ...
                    if (!in_array($tokens[$i][1], $this->v_userinput)) {
                        if ($tokens[$i][1] === 'DIRECTORY_SEPARATOR' || $tokens[$i][1] === 'PATH_SEPARATOR')
                            $value .= '/';
                        else if (strtolower($tokens[$i][1]) === '$componentpath')
                            $value = dirname($file_name);
                        else if ($tokens[$i][1] === '$_SERVER' && $tokens[$i][3][0] === 'DOCUMENT_ROOT')
    ...
    private function highlightLine($line, $tokens = array(), $tainted_vars = array(), $comment = '')
    {
        ...
        for ($i = 0; $i < count($tokens); $i++) {
            ...
            if (is_string($token)) {
                ...
                else if (in_array($token, $this->tokens_type['SPACE_WRAP_STR']) || in_array($token, $this->tokens_type['ARITHMETIC_STR']))
                    $output .= '<span style="color: #007700;">&nbsp;' . $token . '&nbsp;</span>';
                else
                ...
                if (in_array($token[0], $this->tokens_type['SPACE_WRAP']) || in_array($token[0], $this->tokens_type['OPERATOR']) || in_array($token[0], $this->tokens_type['ASSIGNMENT'])) {
                    ...
                    if ($token[0] === T_FUNCTION)
                        ...
                        if ($token[0] === T_VARIABLE && @in_array($token[1], $tainted_vars))
    ...
    private function getVulnNodeTitle($func_name)
    {
        ...
        } else if (isset($this->vuln_func['HTTP_HEADER'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_HEADER_NAME');
        } else if (isset($this->vuln_func['DATABASE'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_DATABASE_NAME');
        } else if (isset($this->vuln_func['FILE_INCLUDE'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_INCLUDE_NAME');
        } else if (isset($this->vuln_func['EXEC'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_EXEC_NAME');
        } else if (isset($this->vuln_func['CODE'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_CODE_NAME');
        } else if (isset($this->vuln_func['POP'][$func_name])) {
            $vulnname = GetMessage('VULNSCAN_POP_NAME');
        } else if (isset($this->vuln_func['OTHER'][$func_name])) {
    ...
    private function getHelp($category)
    {
        ...
        $result .= $this->getVulnNodeDescription($category);
        $result .= '</div>';
        return $result;
    }
    ...
    public function getOutput()
    {
        ...
        if (!empty($output)) {
            foreach ($output as $vuln) {
                ...
                $filename = htmlspecialchars(str_replace(realpath(trim($this->arParams['path'])), '', realpath(trim($vuln->filename))));
                ...
                foreach ($vuln->tainted_vars as $tainted_var_name => $tainted_var) {
                    ...
                    $result .= '<div class="checklist-vulnscan-vulnblock">' . '<div class="checklist-vulnscan-vulnscan-blocktitle">' . $this->getVulnNodeTitle($vuln->name) . '</div>';
...
class CQAACheckListTests
{
            ...
            while (false !== ($file = readdir($handle)))
            {
                if ($file !== '.' && $file !== '..')
                {
                    ...
                    if (!preg_match($skip_preg, str_replace($doc_root,"",$name)))
                        ...
                        } else if (in_array(substr($name, -4), $filetypes))
                        {
                            $arResult[] = $name;
                        }
                }
    ...
    static private function defineScanParams()
    {
        ...
        $arResult['SECURING_FUNCTIONS']['SECURES_ALL'] = array_merge(
            $arResult['SECURING_FUNCTIONS']['XSS'], $arResult['SECURING_FUNCTIONS']['SQL'], $arResult['SECURING_FUNCTIONS']['PREG'], $arResult['SECURING_FUNCTIONS']['FILE'], $arResult['SECURING_FUNCTIONS']['SYSTEM'], $arResult['SECURING_FUNCTIONS']['XPATH']
        );
        ...
        do
        {
            if (is_file($file = $NS['FILE_LIST'][$NS['CUR_FILE_ID']]))
            {
                ...
                if ($result !== false)
                {
                    ...
                    {
                        $NS['VULN_COUNT']+=$scan->vuln_count;
                        $NS['MESSAGE'][$NS['CUR_FILE_ID']] = $scan->getOutput();
                    }
                    $NS['CUR_FILE_ID']++;
                } else
                {
        ...
        if (!($NS['CUR_FILE_ID'] < count($NS['FILE_LIST'])))
        {
            ...
            foreach ($NS['MESSAGE'] as $file_output)
                if (!empty($file_output))
                    if (strpos($arDetailReport, $file_output) === false)
                        $arDetailReport.=$file_output;
                    else $NS['VULN_COUNT']--;
            ...
            $arResult = Array(
                'MESSAGE' => Array(
                    'PREVIEW' => GetMessage('VULNSCAN_FIULECHECKED') . count($NS['FILE_LIST']). GetMessage('VULNSCAN_VULNCOUNTS') .$NS['VULN_COUNT'],
                    'DETAIL' => $arDetailReport
                ),
                'STATUS' => ($NS['VULN_COUNT'] > 0 ? false : true)
            );
        }
        else
        {
