...
class CZip implements IBXArchive
{
    ...
    public function __construct($pzipname)
    {
        ...
        $this->replaceExistentFiles = $replaceExistentFiles;
        $this->checkBXPermissions = $checkBXPermissions;
    ...
    */
    ...
    {
        $this->_errorReset();
        $this->startFile = CBXVirtualIoFileSystem::ConvertCharset($startFile);
        $this->arPackedFiles = array();
        $this->arHeaders = array();

        $isNewArchive = true;
        if (file_exists(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)) && is_file(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)) && ($startFile != ""))
            $isNewArchive = false;
        ...
        else
        {
            ...
            if (($zipfile_tmp = @fopen(CBXVirtualIoFileSystem::ConvertCharset($zipname_tmp), 'wb')) == 0)
            {
                $this->_closeFile();
                $this->_errorLog("ERR_READ_TMP", str_replace("#FILE_NAME#", $zipname_tmp, GetMessage("MAIN_ZIP_ERR_READ_TMP")));
        ...
        foreach ($arFileList as $fullpath)
            $arConvertedFileList[] = CBXVirtualIoFileSystem::ConvertCharset($fullpath);

        if (is_array($arFileList) && count($arFileList)>0)
            $packRes = $this->_processFiles($arConvertedFileList, $this->add_path, $this->remove_path, $this->startFile);
            ...
            for ($i = 0, $counter = 0; $i<sizeof($this->arHeaders); $i++)
            {
                ...
                if ($this->arHeaders[$i]['status'] == 'ok')
                {
                    if (($res = $this->_writeCentralFileHeader($this->arHeaders[$i]))!=1)
                    {
                        fclose($zipfile_tmp);
                        $this->_closeFile();
                        @unlink(CBXVirtualIoFileSystem::ConvertCharset($zipname_tmp));
        ...
        if ($packRes === 'continue')
        {
            $this->startFile = array_pop($this->arPackedFiles);
        }
    ...
    private function _processFiles($arFileList, $addPath, $removePath, $startFile = "")
    {
        ...
        if (!$this->zipfile)
        {
           $this->_arErrors[] = array("ERR_DFILE", GetMessage("MAIN_ZIP_ERR_DFILE"));
            return false;
        }
        ...
        while ($j++ < count($arFileList) && ($this->tempres === "started"))
        {
            ...
            if ($filename == $this->_strArchiveName)
                continue;
            ...
            {
                $this->_arErrors[] = array("ERR_NO_FILE", str_replace("#FILE_NAME#", $filename , GetMessage("MAIN_ZIP_ERR_NO_FILE")));
                continue;
            }
            ...
            else
            {
                ...
                {
                    $this->_arErrors[] = array("ERR_DIR_OPEN_FAIL", str_replace("#DIR_NAME#", $filename, GetMessage("MAIN_ZIP_ERR_DIR_OPEN_FAIL")));
                    continue;
                }
                ...
                while (false !== ($dir = readdir($handle)))
                {
                    if ($dir!="." && $dir!="..")
                    {
                        ...
                        $res = $this->_processFiles($arFileList_tmp, $addPath, $removePath, $this->startFile);
                    }
                }
    ...
    */
    public function SetOptions($arOptions)
    {
        ...
        if (array_key_exists("ADD_PATH", $arOptions))
            $this->add_path = str_replace("\\", "/", strval($arOptions["ADD_PATH"]));
            
        if (array_key_exists("REMOVE_PATH", $arOptions))
            $this->remove_path = str_replace("\\", "/", strval($arOptions["REMOVE_PATH"]));
    ...
    */
    public function GetProperties()
    {
        ...
        if (@is_file(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)))
        {
            if (($this->zipfile = @fopen(CBXVirtualIoFileSystem::ConvertCharset($this->zipname), 'rb')) == 0)
            {
                $this->_errorLog("ERR_READ", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_READ")));
                return 0;
            }
    ...
    private function _checkFormat($level = 0)
    {
        $res = true;
        ...
        if (!is_file(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)))
        {
            $this->_errorLog("ERR_MISSING_FILE", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_MISSING_FILE")));
            return(false);
        }

        if (!is_readable(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)))
        {
            $this->_errorLog("ERR_READ", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_READ")));
            return(false);
        }
    ...
    private function _addData($arFilesList, &$arResultList, &$arParams)
    {
        ...
        if ((!is_file(CBXVirtualIoFileSystem::ConvertCharset($this->zipname))) || (filesize(CBXVirtualIoFileSystem::ConvertCharset($this->zipname)) == 0))
        {
        ...
        $this->_closeFile();
        @fclose($zipfile_tmp);
        @unlink(CBXVirtualIoFileSystem::ConvertCharset($this->zipname));
    ...
    private function _openFile($mode)
    {
        ...
        if ($this->zipfile != 0)
        {
            $this->_errorLog("ERR_OPEN", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_READ_OPEN")));
            return $this->arErrors;
        }

        if (($this->zipfile = @fopen(CBXVirtualIoFileSystem::ConvertCharset($this->zipname), $mode)) == 0)
        {
            $this->_errorLog("ERR_READ_MODE", str_replace(array("#FILE_NAME#","#MODE#"), array($this->zipname, $mode), GetMessage("MAIN_ZIP_ERR_READ_MODE")));
            return $this->arErrors;
        }
    ...
    private function _addFileList($arFilesList, &$arResultList, $addDir, $removeDir, $removeAllDir, &$arParams)
    {
        ...
        for ($j = 0; ($j<count($arFilesList)) && ($res == 1); $j++)
        {
            ...
            if (!file_exists(CBXVirtualIoFileSystem::ConvertCharset($filename)))
            {
                $this->_errorLog("ERR_MISSING_FILE", str_replace("#FILE_NAME#", $filename, GetMessage("MAIN_ZIP_ERR_MISSING_FILE")));
                return $this->arErrors;
            }

            if ((is_file(CBXVirtualIoFileSystem::ConvertCharset($filename))) || ((is_dir(CBXVirtualIoFileSystem::ConvertCharset($filename))) && !$removeAllDir))
            {
            ...
            if (is_dir(CBXVirtualIoFileSystem::ConvertCharset($filename)))
            {
                if ($filename != ".")
                {
                    $path = $filename."/";
                ...
                $hdir  = opendir(CBXVirtualIoFileSystem::ConvertCharset($filename));
                $hitem = readdir($hdir); // '.' directory
                $hitem = readdir($hdir); // '..' directory

                while ($hitem = readdir($hdir))
                {
                    if (is_file(CBXVirtualIoFileSystem::ConvertCharset($path.$hitem)))
                    {
    ...
    private function _addFile($filename, &$arHeader, $addDir, $removeDir, $removeAllDir, &$arParams)
    {
        ...
        if ($arHeader['status'] == 'ok') 
        {
            if (is_file($filename))
            {
                ...
                {
                    $this->_errorLog("ERR_READ", str_replace("#FILE_NAME#", $filename, GetMessage("MAIN_ZIP_ERR_READ")));
                    return $this->arErrors;
                }
        ...
        for ($i = 0, $extractedCounter = 0; $i<$arCentralDirInfo['entries']; $i++)
        {
            ...
            if ($extract)
            {
                ...
                if (@fseek($this->zipfile, $header['offset']))
                {
                    ...
                    $this->_closeFile();
                    $this->_errorLog("ERR_INVALID_ARCHIVE_ZIP", GetMessage("MAIN_ZIP_ERR_INVALID_ARCHIVEzipname"));
                    return $this->arErrors;
                }
                //extract as a string
    ...
    private function _extractFile(&$arEntry, $path, $removePath, $removeAllPath, &$arParams)
    {
        ...
        $arEntry["filename"] = CharsetConverter::ConvertCharset($arEntry["filename"], "cp866", $serverEncoding);
        $arEntry["stored_filename"] = CharsetConverter::ConvertCharset($arEntry["stored_filename"], "cp866", $serverEncoding);
        ...
        {
            if ((HasScriptExtension($arEntry['filename']) || substr(GetFileName($arEntry['filename']), 0, 1)==".") &&
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (($res = $this->_checkDir(CBXVirtualIoFileSystem::ConvertCharset($checkDir), (($arEntry['external']&0x00000010)==0x00000010))) != 1)
                    {
                        //change file status
        ...
        if ($arEntry['status'] == 'ok')
        {
            ...
            if (!(($arEntry['external']&0x00000010)==0x00000010))
            {
                ...
                {   
                    if (($destFile = @fopen(CBXVirtualIoFileSystem::ConvertCharset($arEntry['filename']), 'wb')) == 0)
                    {
                        //change file status
                        $arEntry['status'] = "write_error";
                        return $res;
                ...
                {
                    chmod(CBXVirtualIoFileSystem::ConvertCharset($arEntry['filename']), $arParams['set_chmod']);
                }
            }
        }
    ...
    private function _readEndCentralDir(&$arCentralDir)
    {
        ...
        $size = filesize(CBXVirtualIoFileSystem::ConvertCharset($this->zipname));
        @fseek($this->zipfile, $size);

        if (@ftell($this->zipfile) != $size)
        {
            $this->_errorLog("ERR_ARC_END", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_ARC_END")));
            return $this->arErrors;
        }
        ...
        if ($size > 26)
        {
            ...
            {
                $this->_errorLog("ERR_ARC_MID", str_replace("#FILE_NAME#", $this->zipname, GetMessage("MAIN_ZIP_ERR_ARC_MID")));
                return $this->arErrors;
            }
    ...
    private function _renameTmpFile($source, $dest)
    {
        ...
        if (!@rename(CBXVirtualIoFileSystem::ConvertCharset($source), CBXVirtualIoFileSystem::ConvertCharset($dest)))
        {
            if (!@copy(CBXVirtualIoFileSystem::ConvertCharset($source), CBXVirtualIoFileSystem::ConvertCharset($dest)))
            {
                $res = 0;
            }
            else if (!@unlink(CBXVirtualIoFileSystem::ConvertCharset($source)))
            {
                $res = 0;
            }
        }
    ...
    {
        $this->fileSystemEncoding = strtolower(defined("BX_FILE_SYSTEM_ENCODING") ? BX_FILE_SYSTEM_ENCODING : "");
