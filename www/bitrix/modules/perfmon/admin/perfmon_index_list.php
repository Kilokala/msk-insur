...
if(
	check_bitrix_sessid()
	&& (
		$_REQUEST["action"]=="analyze_start"
		|| $_REQUEST["action"]=="analyze_cont"
	)
)
{
	$etime = time() + 5;

	if($_REQUEST["action"] == "analyze_start")
	{
	...
	if($go)
	{
		...
		?>
		<script>
			<?echo $lAdmin->ActionDoGroup(0, "analyze_cont", bitrix_sessid_get()."&last_id=".$last_id);?>
		</script>
		<?
